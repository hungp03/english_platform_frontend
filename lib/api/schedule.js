...
export async function checkScheduleOverlap(startTime, durationMin, excludeScheduleId) {
  try {
    const params = {
      startTime,
      durationMin
    ...
    // Chỉ thêm param này nếu có giá trị (để tránh gửi null/undefined lên server)
    if (excludeScheduleId) {
      params.excludeScheduleId = excludeScheduleId;
    }

    const res = await api.get(`/api/study-plans/schedules/check-overlap`, {
      params
    });

    if (res.status === 200) {
      // Backend trả về true/false trong res.data.data
      // Ta map nó sang key 'isOverlap' để logic frontend dễ đọc
      return { success: true, isOverlap: res.data?.data };
    }
    return { success: false, error: "Kiểm tra lịch học thất bại" };
  } catch (err) {
    return {
      success: false,
      error: "Kiểm tra lịch học thất bại"
    };
  }
}