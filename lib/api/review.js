...
export async function getPublicCourseReviews(courseId, { page = 1, size = 10 } = {}) {
  try {
...
export async function getInstructorCourseReviews(courseId, { page = 1, size = 10, isPublished, rating } = {}) {
  try {
...
export async function adminDeleteReview(reviewId) {
  try {
    // Lưu ý: Đường dẫn này phải khớp với Backend bạn vừa sửa
    // Nếu bạn sửa backend như tôi gợi ý thì dùng dòng dưới:
    const res = await api.delete(`/api/courses/reviews/${reviewId}/admin`);
    
    // Nếu bạn GIỮ NGUYÊN backend cũ thì dùng dòng này:
    // const res = await api.delete(`/api/courses/${reviewId}/admin`);
    ...
    switch (code) {
      case ErrorCode.EXCEPTION:
        ...
        break;
      case ErrorCode.UNAUTHORIZED:
        errorMessage = "Hết phiên đăng nhập, vui lòng đăng nhập lại.";
        break;
      case ErrorCode.FORBIDDEN:
        errorMessage = "Bạn không có quyền quản trị để xóa đánh giá này.";
        break;
      case ErrorCode.RESOURCE_NOT_FOUND:
        errorMessage = "Đánh giá không tồn tại hoặc đã bị xóa.";
        break;
      default:
        break;
    }
