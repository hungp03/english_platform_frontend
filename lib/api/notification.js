...
export async function removeFcmToken() {
  try {
    ...
    if (token) {
      await api.delete("/api/notifications/remove-token", { data: { token } });
...
// Xóa tất cả FCM Token (khi logout all)
export async function removeAllFcmTokens() {
  try {
    await api.delete("/api/notifications/remove-all-tokens");
    localStorage.removeItem("fcmToken");
    return { success: true };
  } catch (err) {
    console.error("Remove all tokens error", err);
    return { success: false };
  }
}
