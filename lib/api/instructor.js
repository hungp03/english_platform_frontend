...
export async function getInstructorStats() {
  try {
    const res = await api.get("/api/instructors/stats");
    if (res.status === 200) {
      return { success: true, data: res.data?.data };
    }
...
export async function updateInstructorProfile({ bio, expertise, experienceYears, qualification }) {
  try {
    const res = await api.put("/api/instructors/profile", {
      bio,
      expertise,
      experienceYears,
      qualification
