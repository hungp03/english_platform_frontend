...
export async function uploadAttachment(file, title = null) {
  if (!file) {
    return { success: false, error: "Thiếu file" };
  }
  try {
    ...
    const res = await api.post("/api/media/upload-attachment", formData, {
      headers: {
        "Content-Type": "multipart/form-data",
      },
    });
...
export async function getAdminCourseBySlug(slug) {
  if (!slug) {
    return { success: false, error: "Thiếu slug khóa học" };
  }
  try {
    const res = await api.get(`/api/courses/admin/slug/${slug}`);
    if (res.status === 200) {
      return { success: true, data: res.data?.data };
    }
    return { success: false, error: "Không thể lấy thông tin khóa học" };
...
export async function deleteCourse(courseId) {
  ...
  try {
    ...
    if (code === ErrorCode.CANNOT_DELETE) {
      return { success: false, error: "Không thể xóa khóa học vì đã có học viên" };
    }
    return { success: false, error: "Không thể xóa khóa học" };
  }
}
