...
export async function adminLockThread(id) {
  try {
    const res = await api.post(`/api/forum/threads/${id}/admin/lock`);
    if (res.status === 200) {
      return { success: true, data: res.data?.data };
    }
    return { success: false, error: "Không thể khóa chủ đề" };
...
export async function adminUnlockThread(id) {
  try {
    const res = await api.post(`/api/forum/threads/${id}/admin/unlock`);
    if (res.status === 200) {
      return { success: true, data: res.data?.data };
    }
    return { success: false, error: "Không thể mở khóa chủ đề" };
...
/**
 * Lấy danh sách các chủ đề đã lưu (Yêu thích) 
 */
export async function getSavedThreads(params = {}) {
  try {
    const res = await api.get("/api/forum/threads/saved", { params });
    if (res.status === 200) {
      return { success: true, data: res.data?.data };
    }
    return { success: false, error: "Không thể lấy danh sách đã lưu" };
  } catch (err) {
    console.error(err);
    const code = err?.response?.data?.code;
    let errorMessage = "Không thể lấy danh sách đã lưu";
    ...
    switch (code) {
      case ErrorCode.EXCEPTION:
        ...
        break;
      case ErrorCode.UNAUTHORIZED:
        errorMessage = "Bạn cần đăng nhập để xem danh sách đã lưu.";
        break;
      default:
        break;
    }
...
/**
 * Lưu/Bỏ lưu một chủ đề (Yêu thích) 
 */
export async function toggleSaveThread(id) {
  try {
    const res = await api.post(`/api/forum/threads/${id}/save`);
    if (res.status === 200) {
      return { success: true };
    }
    return { success: false, error: "Không thể thay đổi trạng thái lưu" };
  } catch (err) {
    console.error(err);
    const code = err?.response?.data?.code;
    let errorMessage = "Không thể thay đổi trạng thái lưu";
    ...
    switch (code) {
      case ErrorCode.EXCEPTION:
        ...
        break;
      case ErrorCode.UNAUTHORIZED:
        errorMessage = "Bạn cần đăng nhập để lưu bài viết.";
        break;
      case ErrorCode.RESOURCE_NOT_FOUND:
        errorMessage = "Không tìm thấy chủ đề.";
        break;
...
export async function reportReview(reviewId, reason) {
  try {
    const res = await api.post(`/api/forum/reviews/${reviewId}/reports`, { reason });
    if (res.status === 201 || res.status === 200) {
      return { success: true, data: res.data?.data };
    }
    return { success: false, error: "Không thể báo cáo đánh giá" };
  } catch (err) {
    console.error(err);
    const code = err?.response?.data?.code;
    let errorMessage = "Không thể báo cáo đánh giá";
    ...
    switch (code) {
      case ErrorCode.EXCEPTION:
        ...
        break;
      case ErrorCode.UNAUTHORIZED:
        errorMessage = "Bạn cần đăng nhập để báo cáo.";
        break;
      default:
        break;
    }
