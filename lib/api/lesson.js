...
export const attachMediaToLesson = async (moduleId, lessonId, assetId) => {
  if (!moduleId || !lessonId || !assetId) {
    return { success: false, error: "Thiếu thông tin cần thiết" }
  }

  try {
    const res = await api.post(`/api/modules/${moduleId}/lessons/${lessonId}/assets/${assetId}`)
    if (res.status === 200) {
      return { success: true, data: res.data?.data }
    } else {
      return {
        success: false,
        error: res.data?.message || "Không thể thêm media vào bài học",
      }
    }
  } catch (err) {
    const code = err?.response?.data?.code;
    if (code === ErrorCode.RESOURCE_NOT_FOUND) {
      return { success: false, error: "Không tìm thấy bài học hoặc media" };
    }
    if (code === ErrorCode.FORBIDDEN) {
      return { success: false, error: "Bạn không có quyền chỉnh sửa bài này" };
    }
    return {
      success: false,
      error: "Lỗi khi thêm media vào bài học",
    }
  }
}

export const detachMediaFromLesson = async (moduleId, lessonId, assetId) => {
  if (!moduleId || !lessonId || !assetId) {
    return { success: false, error: "Thiếu thông tin cần thiết" }
  }

  try {
    const res = await api.delete(`/api/modules/${moduleId}/lessons/${lessonId}/assets/${assetId}`)
    if (res.status === 200) {
      return { success: true, data: res.data?.data }
    } else {
      return {
        success: false,
        error: res.data?.message || "Không thể xóa media khỏi bài học",
      }
    }
  } catch (err) {
    const code = err?.response?.data?.code;
    if (code === ErrorCode.RESOURCE_NOT_FOUND) {
      return { success: false, error: "Không tìm thấy media trong bài học" };
    }
    if (code === ErrorCode.FORBIDDEN) {
      return { success: false, error: "Bạn không có quyền chỉnh sửa bài này" };
    }
    return {
      success: false,
      error: "Lỗi khi xóa media khỏi bài học",
    }
  }
}